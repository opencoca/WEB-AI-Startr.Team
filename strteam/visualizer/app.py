...
# to run a project we must call the strteam module
# python -m strteam [-h] [--config CONFIG] [--org ORG] [--task TASK] [--name NAME] [--model MODEL] [--path PATH]
import subprocess

@app.route("/send_project", methods=["POST"])
def send_project():
    data = request.get_json()
    cmd = ["python3", "-m", "strteam"]
